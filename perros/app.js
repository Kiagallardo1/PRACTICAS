...
const changePerrito=async (raza)=>{
    const api=await fetch(`https://dog.ceo/api/breed/${raza}/images/random/15`);
    const response=await api.json();
    console.log(response.message);
    let divPerritos=document.querySelector("#perrito");
    data=response.message;
    divPerritos.inerHTML="";
    data.map(imgPerrito=>{
